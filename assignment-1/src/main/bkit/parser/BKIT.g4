...
list_var 			:variable_id (EQUAL value)? (COMMA variable_id (EQUAL value)?)*;
variable_id			:IDENTIFIER | (IDENTIFIER ('[' INTEGER ']')+);
value 				:primary_type | array;

array 				:LCB (array | (primary_type | IDENTIFIER) (COMMA (primary_type | IDENTIFIER))* | COMMA)* RCB;
primary_type 		: ((SUB_OP_INT | ADD_OP_INT)? INTEGER) | (SUB_OP_INT | ADD_OP_INT)? FLOAT | STRING | TRUE | FALSE | array;
...
if_sta 				:IF exp THEN list_sta (ELSEIF exp THEN list_sta)* (ELSE list_sta)? ENDIF DOT;
...
do_while_sta 		:DO variable_dec* statement* WHILE exp ENDDO DOT; // ? list_sta isn't used here
...
left_operand 		:IDENTIFIER | (IDENTIFIER index_operators);
...
exp6 					:name_var index_operators
						|fuction_call
						;
name_var 				:IDENTIFIER | fuction_call; //need to think carefully
index_operators 		:(LSB exp RSB)+
						;

fuction_call 	 		:IDENTIFIER LP list_exp? RP  // exp7
						|exp8
						;
exp8 					:primary_type | IDENTIFIER | (LP exp RP);
...
INTEGER 								:'0' | NZERO_DIGIT DIGIT* | (HEXA NZERO_DIGIT DIGIT*) | (OCTAL NZERO_DIGIT DIGIT*);//need to think carefully
