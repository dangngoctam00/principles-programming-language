...
def main(argv):
    ...
    if len(argv) < 1:
        ...
        if len(argv) < 2:
            ...
            test(suite)
        elif argv[1] == 'ASTGenSuiteLT':
            from ASTGenSuiteLT import ASTGenSuite
            suite = unittest.makeSuite(ASTGenSuite)
            test(suite)
        elif argv[1] == 'ASTGenSuiteNT':
            from ASTGenSuiteNT import ASTGenSuite
            suite = unittest.makeSuite(ASTGenSuite)
            test(suite)
        elif argv[1] == 'ASTDump':
            from ASTDump import ASTGenSuite
            suite = unittest.makeSuite(ASTGenSuite)
            test(suite)
        else:
            printUsage()
